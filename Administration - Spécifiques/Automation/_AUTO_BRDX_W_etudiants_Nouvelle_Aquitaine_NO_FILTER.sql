...
WHERE 
    (
        ...
        AND (
            TRIM(SUBSTR(ExtractValue(bm.metadata, '//datafield[@tag="100"]/subfield[@code="a"]'), 10, 4)) != "" /*date_100*/
            OR TRIM(REGEXP_SUBSTR(bi.publicationyear, "\\d{4}")) != "" /*date_mod*/
        )
    )

/* Rapport ID (test) : 
Rapport ID (prod) : 1504
